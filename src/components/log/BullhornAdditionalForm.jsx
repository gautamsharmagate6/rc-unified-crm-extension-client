        ...
        chrome.storage.local.get({ bullhornDefaultActionCode: '' },
            (items => {
                if (items.bullhornDefaultActionCode && additionalFormInfo.actions.some(i => i.title === items.bullhornDefaultActionCode)) {
                    ...
                    setSubmission({
                        commentAction: items.bullhornDefaultActionCode,
                        noteActions: items.bullhornDefaultActionCode
                    })
                }
                else {
                    setAdditionalDropdownSelection(additionalFormInfo.actions[0].id);
                    setSubmission({
                        commentAction: additionalFormInfo.actions[0].id,
                        noteActions: additionalFormInfo.actions[0].id
                    });
                }
            }));
    }, [additionalFormInfo])
            ...
            {additionalFormInfo != null &&
                <DropdownList
                    ...
                    onSelected={(selection) => {
                        ...
                        selection ? setSubmission({ 
                            commentAction: selection,
                            noteActions: selection
                         }) : setSubmission({});
                    }} />
            }
        </div>
